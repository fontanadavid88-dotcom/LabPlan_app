                                    ...
                                    return (<React.Fragment key={date.toISOString()}>
                                        <PersonnelStatusCell
                                            ...
                                            onEdit={(e) => {
                                                ...
                                                // Using flushSync to ensure DOM is updated before we read dimensions
                                                flushSync(() => {
...
const DataManagementView: React.FC<DataManagementViewProps> = ({ data, onDataChange }) => {
    ...
    const CrudComponent = <T extends { id: string } & Record<string, any>>({ name, items, setItems, fields, data }: {
        name: string;
        items: T[];
                        ...
                        {items.map(item => (
                            <tr key={item.id}>
                                ...
                                <td style={{textAlign: 'right'}}>
                                    ...
                                    <button className="btn btn-danger" style={{marginLeft: '0.5rem'}} onClick={() => setItems(items.filter(i => i.id !== item.id))}>Elimina</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
    ...
    return (
        <div>
            ...
            {tab === 'campaigns' && <CrudComponent name="Campagna" items={data.campaigns} setItems={(items) => onDataChange('campaigns', items)} fields={[
                { key: 'name', label: 'Nome', type: 'text' },
                    ...
                    {data.absences.map(absence => (
                        <tr key={absence.id}>
                            ...
                            <td style={{textAlign: 'right'}}>
                                ...
                                <button className="btn btn-danger" style={{marginLeft: '0.5rem'}} onClick={() => onDataChange('absences', data.absences.filter(a => a.id !== absence.id))}>Elimina</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
